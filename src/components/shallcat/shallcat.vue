<template>
	<section>
      <div class="shoplist">
        <!-- 购物车结果 -->
      <div class="shall_content">
          <ul class="goods_area">
              <li @click="togglelist()">
                <span class="one_li" :class="{'heightliht':totalCount>0}">{{totalCount}}</span>
        ...
        <transition name="slideup">
          ...
          <div class="scroll" ref="goods-content">
...
export default {
  ...
  methods: {
    ...
    play () {
      ...
      window.alert(`需要支付${this.totalPrice}元，^_^`)
    }
  },
  computed: {
    totalPrice () {
    ...
    listShow () {
      ...
      if (show) {
        this.$nextTick(() => {
          if (!this.listscroll) {
            this.listscroll = new BScroll(this.$refs.goodsContent, {
              click: true,
              probeType: 3
            })
          } else {
...
<style scoped>
	...
	.shoplist ul{font-family: "微软雅黑"; display: flex;}
	...
	.good_warp{position: fixed;bottom: 2.048rem;left: 0;z-index:-1;max-height:218px;background-color: #fff;width: 100%;}
  ...
  .scroll .price span{ color: red; }
  .scroll li:last-child{ border-bottom: none; }
</style>